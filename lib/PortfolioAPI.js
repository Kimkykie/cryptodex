...
class PortfolioAPI {
  ...
  async getPortfolioTokenOnly(tokenOption) {
    try {
      if (!isValidToken(tokenOption)) {
        throw new Error(
          `${tokenOption} is not a valid token. Please use BTC,ETH or XRP`
        );
      }
      console.log(
      ...
      await readCSVLatestPortfolio().then((result) => {
        if (result) {
          tokenLatestPorfolioValue = result.find(
            (transaction) => transaction.token === tokenOption
          );
  ...
  async getPortfolioDateAndToken(tokenOption, dateOption) {
    try {
      ...
      await readCSVDatePortfolio(dateOption).then((result) => {
        if (result) {
          let tokenResult = result.filter((transaction) => {
            return transaction.token === tokenOption;
          });
          tokenResult.forEach((transaction) => {
            output += `${transaction.token.yellow} : ${
              formatter.format(transaction.amount * res.data.USD).green
            }\n`;
          });
        }
      });
